  ...
  return message.replace(jsonRegex, (match, jsonString) => {
    try {
      ...
      data.listings.forEach((listing) => {
        ...
        const formattedSqFt = new Intl.NumberFormat("en-US").format(
          listing.squareFeet
        );
        ...
        listingsHTML += `
          <div class="property-card bg-zinc-950 border pt-1 px-1 border-zinc-800 overflow-hidden shadow-md hover:shadow-lg transition-shadow">
            ...
            <div class="property-details p-4">
              ...
              <div class="property-address text-gray-300 mb-3">${
                listing.address
              }, ${listing.city}</div>
              <div class="property-specs flex text-sm text-gray-400 mb-3">
                <div class="beds mr-3">
                  <span class="font-bold">${listing.bedrooms}</span> bed${
          listing.bedrooms !== 1 ? "s" : ""
        }
                </div>
                <div class="baths mr-3">
                  <span class="font-bold">${listing.bathrooms}</span> bath${
          listing.bathrooms !== 1 ? "s" : ""
              ...
              <a 
                ...
                class="view-listing-btn block w-full text-center bg-red-600 hover:bg-red-700 text-white py-2  transition-colors"
              >
                View Listing
              </a>
            </div>
...
const Message = ({ messageType, message }) => {
  ...
  if (messageType === "ai" && cleanedMessage.includes("```json")) {
    ...
    const completeJsonBlock = cleanedMessage.match(
      /```json\s*([\s\S]*?)\s*```/
    );
        ...
        <div
          className={`px-4 py-3 text-[14px] flex flex-col gap-3 ${
            messageType === "ai"
              ...
              : "bg-red-00 text-gray-200 w-fit max-w-9/10 ml-auto break-words animate-slide-in-right shadow-md"
          }`}
        >
          {messageType === "ai" && (
            <div className="text-sm text-gray-300 overline decoration-red-500">
              AI
            </div>
          )}
